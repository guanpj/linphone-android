 ...
 */
public class LinphoneMiniPreferences
{
	private static final int LINPHONE_CORE_RANDOM_PORT = -1;
	private static LinphoneMiniPreferences instance;
	private Context mContext;
	private String basePath;

	public static final synchronized LinphoneMiniPreferences instance() {
		if (instance == null) {
			instance = new LinphoneMiniPreferences();
		}
		return instance;
	}

	private LinphoneMiniPreferences() {
	...
	private String getString(int key) {
		if (mContext == null && LinphoneMiniManager.isInstanciated()) {
			mContext = LinphoneMiniManager.getInstance().getContext();
		}
	...
	private LinphoneCore getLc() {
		if (!LinphoneMiniManager.isInstanciated())
			return null;

		return LinphoneMiniManager.getLcIfManagerNotDestroyedOrNull();
	}
	...
	public LpConfig getConfig() {
		...
		if (!LinphoneMiniManager.isInstanciated()) {
			...
			return LinphoneCoreFactory.instance().createLpConfig(LinphoneMiniManager.getInstance().mLinphoneConfigFile);
		}
		return null;
	}
	...
	public void setAccountPassword(int n, String password) {
		if(getAccountDomain(n) != null && getAccountUsername(n) != null) {
			...
			LinphoneMiniManager.getLc().addAuthInfo(authInfo);
		}
	}
